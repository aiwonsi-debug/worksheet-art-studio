...
import { ASSET_KINDS, buildWorksheetAssetPrompt, decodeImageDataUrl, normalizeAssetName, normalizeWorksheetTitle } from "./worksheetUtils";
...
export const appRouter = router({
  ...
  asset: router({
    ...
    save: protectedProcedure.input(z.object({ kind: assetKindInput, name: z.string().min(1).max(1200), dataUrl: z.string().min(16).max(17_000_000), prompt: z.string().max(1200).optional() })).mutation(async ({ ctx, input }) => {
      ...
      const asset = await db.createAsset({ userId: ctx.user.id, kind: input.kind, name: normalizeAssetName(input.name), prompt: input.prompt ?? null, storageKey: key, url, mimeType });
      if (!asset) throw new Error("Asset could not be saved.");
      return asset;
    }),
    generate: protectedProcedure.input(z.object({ kind: z.enum(["clipart", "border", "header"]), name: z.string().min(1).max(1200), prompt: z.string().min(3).max(1200), style: z.string().max(320).optional() })).mutation(async ({ ctx, input }) => {
      ...
      const asset = await db.createAsset({ userId: ctx.user.id, kind: input.kind, name: normalizeAssetName(input.name), prompt: input.prompt, storageKey: result.url.replace("/manus-storage/", ""), url: result.url, mimeType: "image/png" });
