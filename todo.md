...
- [ ] Fix production cache busting: production edge rewrites all non-/api paths (including /__manus__/version.json) to the SPA index.html, so the static version.json never reaches the client; point enforceFreshBundle() at an Express API route (/api/app-version) that returns the deployed checkpoint version, and have the server read the same client/public/__manus__/version.json the build embeds.
- [ ] Add regression tests for the /api/app-version endpoint (returns {version} matching the embedded marker source) and re-run the cacheBust suite with the new fetch target.
- [ ] Verify the new endpoint works on the dev server and in production, publish, sync to GitHub, and confirm end-to-end stale-cache recovery behavior.
