...
- [ ] User reports mobile still shows light background, toolbar overlap, can't draw outside: verify the deployed production assets match the latest checkpoint, check for service worker / stale bundle caching on mobile Brave, force cache-busting so the fresh build loads, apply/verify the toolbar pass-through fix in a real browser session at mobile width, publish, and sync to GitHub.
