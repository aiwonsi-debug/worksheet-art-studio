...
function parseCanvasData(value: string): WorksheetCanvasState {
  try {
    ...
    if (Array.isArray(data.layers)) return { layers: data.layers, transparentBackground: Boolean(data.transparentBackground), darkPaper: Boolean(data.darkPaper) };
...
export default function Home() {
  ...
  const activeBrushPreset = useMemo(() => brushPresets.find((preset) => isActiveBrushPreset(preset, { color: brushColor, size: brushSize, opacity: brushOpacity }))?.id ?? null, [brushColor, brushOpacity, brushSize]);
  ...
  function applyBrushPreset(presetId: (typeof brushPresets)[number]["id"]) {
    ...
    setTool("brush");
    toast.message(`${preset.label} brush selected.`);
  }
    ...
    <main className="studio-main art-studio-main">
      ...
      <section className={`art-workspace ${panelVisible ? "has-panel" : ""}`}>
        <aside className="art-tool-rail" aria-label="Drawing tools">
          ...
          <div className="art-rail-bottom"><Tooltip><TooltipTrigger asChild><button className="art-rail-action" onClick={() => setCanvas((current) => ({ ...current, transparentBackground: !current.transparentBackground }))} aria-label="Toggle canvas background"><span className={`transparency-icon ${canvas.transparentBackground ? "is-on" : ""}`}/></button></TooltipTrigger><TooltipContent>{canvas.transparentBackground ? "Transparent background" : "White background"}</TooltipContent></Tooltip><Tooltip><TooltipTrigger asChild><button className={`art-rail-action ${canvas.darkPaper ? "is-on" : ""}`} onClick={() => setCanvas((current) => ({ ...current, darkPaper: !current.darkPaper }))} aria-label="Toggle dark paper"><Moon size={17}/></button></TooltipTrigger><TooltipContent>{canvas.darkPaper ? "Dark paper" : "Light paper"}</TooltipContent></Tooltip><Tooltip><TooltipTrigger asChild><button className={`art-rail-action ${penDetected ? "is-detected" : ""}`} onClick={() => setPenSetupOpen(true)} aria-label="Pen display setup"><PenLine size={18}/></button></TooltipTrigger><TooltipContent>{penDetected ? "Pen detected" : "Pen display setup"}</TooltipContent></Tooltip></div>
        </aside>
        <div className="art-canvas-zone">
          ...
          <div className="art-canvas-meta"><span>Letter • 8.5 × 11 in</span><span>{canvas.layers.length} {canvas.layers.length === 1 ? "layer" : "layers"}</span><span>{canvas.transparentBackground ? "Transparency on" : canvas.darkPaper ? "Dark paper" : "White paper"}</span></div>
        </div>
