 ...
 * real brush mark. The outline traces both edges of the stroke spine using
 * tangent normals derived from the same shared join points as the legacy
 * segment renderer, closes with rounded tapered caps at each end, and fills
 * as a single solid shape. Paths stay continuous at any smoothing strength.
 */
export function ribbonStrokePath(points: StrokePoint[], smoothing = 1, taper = 0.55): string {
  ...
  for (let index = 1; index < sampleCount; index += 1) {
    path.push(`L ${leftEdge[index].x} ${leftEdge[index].y}`);
  }
  const endRadius = widths[sampleCount - 1] / 2;
  path.push(`A ${endRadius} ${endRadius} 0 0 0 ${rightEdge[sampleCount - 1].x} ${rightEdge[sampleCount - 1].y}`);
  for (let index = sampleCount - 2; index >= 0; index -= 1) {
    path.push(`L ${rightEdge[index].x} ${rightEdge[index].y}`);
  }
  const startRadius = widths[0] / 2;
  path.push(`A ${startRadius} ${startRadius} 0 0 0 ${leftEdge[0].x} ${leftEdge[0].y} Z`);
  return path.join(" ");
}
